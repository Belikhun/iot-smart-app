...
class Scrollable {
	...
	constructor(container, {
		...
		this.content.addEventListener("scroll", (e) => {
			if (!this.running) {
				this.current = this.target = (this.horizontal)
		...
		this.content.addEventListener("wheel", (event) => {
			...
			if (this.scrollout) {
				if (isPrecise) {
					if (this.horizontal && !event.deltaX)
						return;

					if (!this.horizontal && !event.deltaY)
						return;
				}
	...
	scrollTo(position = 0, {
		...
		return new Promise((resolve) => {
			...
			new Animator(duration, timing, (t) => {
				const c = begin + (position - begin) * t;
				this.content[horizontal ? "scrollLeft" : "scrollTop"] = c;
			}).onComplete(() => resolve());
		});
	}
